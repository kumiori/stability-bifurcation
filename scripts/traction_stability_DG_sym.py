...
def numerical_test(
    ...
    bcs_u = [dolfin.DirichletBC(V_u.sub(0), dolfin.Constant(0), left, method='geometric'),
             dolfin.DirichletBC(V_u.sub(0), ut, right, method='geometric'),
             dolfin.DirichletBC(V_u, (0, 0), left_bottom_pt, method="geometric")] # method="pointwise")
    ...
    def consistency_energy(u, alpha, k_res=k_res):
        ...
        return -inner(dot(avg(sigma),normal('+')), jump(u))*dS
