...
def traction_test(
    ...
    for it, load in enumerate(load_steps):
        ...
        if (mineig + Deltav)*(lmbda_min_prev+dolfin.DOLFIN_EPS) < 0 and not bifurcated:
            ...
            # time_data_i["bifurcation_load"] = 
            modes = np.where(stability.eigs < 0)[0]
            with file_bif as f:
                # import pdb; pdb.set_trace()
                # np.where(stability.eigs)
                # leneigs = min([len([stability.eigs[i]]) for i in range(len(stability.eigs)) if stability.eigs[i] is not None])
                leneigs = len(modes)
                # print(stability.eigs)
                maxmodes = min(3, leneigs)

                for n in range(maxmodes):
                    print(n)
                    mode = dolfin.project(stability.linsearch[n]['beta_n'], V_alpha)
                    mode.rename('beta_{}'.format(n), 'beta_{}'.format(n))
                    # import pdb; pdb.set_trace()
                    f.write_checkpoint(mode, 'mode {}'.format(n), bifn)

                    asd = dolfin.Function(V_alpha)
                    f.read_checkpoint(asd, 'mode 0', bifn)

                    print('saved beta_{}'.format(n))
